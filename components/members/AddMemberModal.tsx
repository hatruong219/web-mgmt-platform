...
export function AddMemberModal({ siteId, siteName, isSuperAdmin }: AddMemberModalProps) {
  const [isOpen, setIsOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [success, setSuccess] = useState<{ inviteUrl: string } | null>(null)

  const allowedRoles: SiteRole[] = isSuperAdmin 
    ? ['admin', 'editor', 'viewer'] 
    : ['editor', 'viewer']
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ fontSize: '1.125rem', fontWeight: 600 }}>Thêm Member</h2>
                <p style={{ fontSize: '0.8125rem', color: 'hsl(var(--muted-foreground))', marginTop: '0.25rem' }}>
                  Mời người dùng vào {siteName}
                </p>
              </div>
              <button
                onClick={handleClose}
              ...
              <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                ...
                >
                  <input
                    type="text"
                    readOnly
                    value={`${typeof window !== 'undefined' ? window.location.origin : ''}${success.inviteUrl}`}
                    style={{
                      flex: 1,
                      background: 'transparent',
                      border: 'none',
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))', marginTop: '0.5rem' }}>
                    {!isSuperAdmin && 'Site Admin chỉ có thể invite Editor hoặc Viewer'}
                  </p>
                </div>
